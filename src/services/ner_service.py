...
class NERService:
    ...
    def __init__(self, model_name: str = "pt_core_news_sm"):
        ...
        try:
            ...
            self.logger.warning(f"Could not load {model_name}, using blank Portuguese model")
            self.nlp = Portuguese()
            ...
            if "ner" not in self.nlp.pipe_names:
                try:
                    ...
                    self.logger.error(f"Could not add NER component: {e}")
        
        self.logger.info(f"NER Service initialized with pipeline: {self.nlp.pipe_names}")
    ...
    def _extract_entities(self, text: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for ent in doc.ents:
                ...
                pii_type = self._ENTITY_TYPE_MAPPING.get(ent.label_, f"ENTIDADE_{ent.label_}")
                ...
                false_positives = {
                    'oi', 'olá', 'ei', 'cpf', 'cnpj', 'email', 'telefone', 'rg', 'cep', 
                    'use', 'clt', 'cargo', 'setor', 'cnh', 'dados', 'nome', 'completo',
                    'matrícula', 'nascido', 'funcionário', 'endereço', 'telefone', 'salário',
                    'histórico', 'login', 'rede', 'acesso', 'ip', 'gestão', 'assinatura',
                    'artigo', 'lei', 'código', 'sistema', 'empresa', 'trabalho', 'operações',
                    'manutenção', 'técnico', 'jr', 'senior', 'pleno', 'analista', 'coordenador',
                ...
                if ('[' in ent.text or ']' in ent.text or '_' in ent.text or 
                    ent.text.isupper() or ent.text.isdigit()):
                    continue
                
                # Skip single words that are too generic or common
                if (len(ent.text.strip()) <= 2 or 
                    text_lower in {'de', 'da', 'do', 'em', 'na', 'no', 'e', 'é', 'meu', 'minha', 'sua', 'seu',
                                   'para', 'com', 'por', 'sem', 'uma', 'um', 'das', 'dos', 'pela', 'pelo'}):
                    continue
                
                # Skip entities that contain numbers (likely codes, IDs, etc.)
                if any(c.isdigit() for c in ent.text) and not self._is_valid_name_with_numbers(ent.text):
                    continue
                ...
                self.logger.debug(
                    f"Entity detected: '{ent.text}' ({ent.label_} -> {pii_type}) "
                    f"at span ({ent.start_char}, {ent.end_char})"
                )
    ...
    def _extract_entities_avoiding_placeholders(self, text: str, placeholders: List[str]) -> List[Dict[str, Any]]:
    ...
    def _is_valid_entity_for_type(self, text: str, label: str) -> bool:
        ...
        job_patterns = ['cargo', 'função', 'técnico', 'analista', 'gerente', 
                       'coordenador', 'assistente', 'supervisor', 'estagiário',
                       'manutenção', 'júnior', 'jr', 'senior', 'sr', 'pleno']
    ...
    def filter_by_ner(
        self, text: str, confidence_threshold: float = 0.5, existing_placeholders: List[str] = None
        ...
        if existing_placeholders:
            entities = self._extract_entities_avoiding_placeholders(text, existing_placeholders)
        else:
            entities = self._extract_entities(text)
        
        # Filter by confidence threshold
        entities = [e for e in entities if e.get("confidence", 1.0) >= confidence_threshold]
        ...
        for i, entity in enumerate(entities):
            ...
            self.logger.debug(f"Replaced '{original_value}' with '{placeholder}'")
            
        # Reverse the list to maintain original order
        pii_mappings.reverse()
        
        self.logger.info(f"NER filtering completed. Found {len(pii_mappings)} entities")
    ...
    def restore_from_ner(
        ...
        for mapping in sorted(pii_mappings, key=lambda x: x.placeholder):
            restored_text = restored_text.replace(mapping.placeholder, mapping.original_value)
            self.logger.debug(f"Restored '{mapping.placeholder}' to '{mapping.original_value}'")
